  ...
  return (
    <CustomModal open={open} onClose={onClose} title="Configurar entrenamiento" maxWidth="max-w-2xl">
      <div className="space-y-5">
        <div>
          <p className="mb-3 text-xs font-semibold text-slate-600">Selecciona un preset de entrenamiento:</p>
            ...
            {PRESETS.map((p) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="mb-2"><p.icon size={28} className="text-brand-600" /></div>
                  <div className={`mb-0.5 text-sm font-bold ${isSelected ? "text-brand-700" : "text-slate-800"}`}>
                    {p.label}
                  </div>
                  <div className="mb-2 text-[10px] leading-tight text-slate-500">{p.description}</div>
                  <div className="mb-1 space-y-0.5 text-[10px] text-slate-400">
                    {Object.entries(p.params).map(([key, val]) => (
                      <div key={key}>
                        {OVERRIDE_LABELS[key] ?? key}: <span className="font-medium text-slate-600">{String(val)}</span>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="mt-1 text-[9px] text-slate-400 italic">{p.tip}</div>
                </button>
              );
            })}
          </div>
        ...
        <button
          ...
          className="flex w-full items-center justify-between rounded-lg border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-600 hover:bg-slate-50"
        >
