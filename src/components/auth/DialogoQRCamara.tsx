...
export const DialogoQRCamara = ({
  ...
  const [codigoVinculacion, setCodigoVinculacion] = useState<string | null>(
    null,
  );
  ...
  useEffect(() => {
    if (!open) return;
    setError(null);
    fetch("/auth/generar-vinculacion", {
      method: "POST",
      headers: {
        Authorization: `Bearer ${localStorage.getItem("epp_token") || ""}`,
        "Content-Type": "application/json",
      },
      body: JSON.stringify({ id_camara: camaraId }),
    })
      .then((res) => res.json())
      .then((data) => {
        if (data.ok && data.data?.codigo_vinculacion) {
          setCodigoVinculacion(data.data.codigo_vinculacion);
        } else {
          setError(data.msg || "Error al generar código de vinculación");
        }
      })
      .catch(() => {
        setError("Error de conexión al generar código de vinculación");
      });
  }, [open, camaraId]);

  const qrUrl = useMemo(() => {
    const code = codigoVinculacion || "";
    const origin = publicUrl || window.location.origin;
    return `${origin}/phone-camera/${camaraId}?code=${code}`;
  }, [camaraId, publicUrl, codigoVinculacion]);
  ...
  return (
    <CustomModal open={open} onClose={onClose} title="Vincular teléfono">
      <div className="flex flex-col items-center gap-5 py-2">
        ...
        {error && (
          <div className="w-full bg-red-50 border border-red-200 rounded-lg p-3 text-[12px] text-red-700 text-center">
            {error}
          </div>
        )}
